...
import type { SongImportSchema } from "./useSongImport";
import { useSongImport } from "./useSongImport";
import * as sampleCsv from "./songs_export.csv?raw";
...
export const SongImportButton = () => {
  ...
  return (
    <>
      ...
      <SongImportDialog open={modalOpen} onClose={handleCloseModal} />
    </>
  );
};

const SAMPLE_URL = `data:text/csv;name=crm_songs_sample.csv;charset=utf-8,${encodeURIComponent(
  sampleCsv.default,
)}`;

type SongImportModalProps = {
  open: boolean;
  onClose(): void;
};

export function SongImportDialog({ open, onClose }: SongImportModalProps) {
        ...
        <Form className="flex flex-col gap-4">
          ...
          <div className="flex flex-col space-y-2">
            {importer.state === "running" && (
              <div className="flex flex-col gap-2">
                ...
                <div className="text-sm">
                  ...
                  songs, with <strong>{importer.errorCount}</strong> errors.
            ...
            {importer.state === "complete" && (
              <Alert>
                <AlertDescription>
                  Songs import complete. Imported {importer.importCount} songs,
                ...
                <Alert>
                  <AlertDescription className="flex flex-col gap-4">
                    ...
                    <Button asChild variant="outline" size="sm">
                      <Link to={SAMPLE_URL} download={"crm_songs_sample.csv"}>
                        Download CSV sample
                      </Link>
                    </Button>{" "}
                  </AlertDescription>
